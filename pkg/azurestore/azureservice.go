...
// New Azure service for communication to Azure Blob Storage API
func NewAzureService(settings *AzureConfig) (*AzService, error) {
	...
	return &AzService{
		...
		MaxAppendBlobSize:     azblob.AppendBlobMaxAppendBlockBytes,
...
// ==== Core Functions ====
func Download(ctx context.Context, blob azblob.BlockBlobURL) (data []byte, err error) {
	defer func() {
		if r := recover(); r != nil {
			switch x := r.(type) {
			case string:
...
func Upload(ctx context.Context, body io.ReadSeeker, blob azblob.BlockBlobURL) (err error) {
...
func Delete(ctx context.Context, blob azblob.BlockBlobURL) error {
...
func CommitUpload(ctx context.Context, blob azblob.BlockBlobURL, indexes []int) error {
...
func (infoBlob *InfoBlob) UploadInfoFile(ctx context.Context, body io.ReadSeeker) (err error) {
	return Upload(ctx, body, infoBlob.Blob)
}

func (infoBlob *InfoBlob) Download(ctx context.Context) (data []byte, err error) {
	return Download(ctx, infoBlob.Blob)
}

func (infoBlob *InfoBlob) Delete(ctx context.Context) (err error) {
	return Delete(ctx, infoBlob.Blob)
}

// ==== File Blob Functions ====
func (fileBlob *FileBlob) Download(ctx context.Context) (data []byte, err error) {
	return Download(ctx, fileBlob.Blob)
}

func (fileBlob *FileBlob) Initialise(ctx context.Context) error {
	return Upload(ctx, nil, fileBlob.Blob)
}
...
func (fileBlob *FileBlob) Upload(ctx context.Context, body io.ReadSeeker, index int) error {
	_, err := fileBlob.Blob.StageBlock(ctx, blockIDIntToBase64(index), body, azblob.LeaseAccessConditions{}, nil)
	return err
	//return CommitUpload(ctx, fileBlob.Blob, []int{index})
	// return UploadChunks(ctx, body, fileBlob.Blob)
}

func (fileBlob *FileBlob) Delete(ctx context.Context) error {
	return Delete(ctx, fileBlob.Blob)
}

func (fileBlob *FileBlob) CommitUpload(ctx context.Context, indexes []int) error {
	return CommitUpload(ctx, fileBlob.Blob, indexes)
}
...
func (fileBlob *FileBlob) GetBlockPosition(ctx context.Context) ([]int, int64, error) {
	...
	getBlock, err := fileBlob.Blob.GetBlockList(ctx, azblob.BlockListAll, azblob.LeaseAccessConditions{})
	if err != nil {
